...
in response to requests. A conventional mutable data structure, such as a
resizable array, is a simple example of a stateful system, and is the one we
shall study below. The definition covers much larger systems too - a network
service, or the concurrent servers of [Chapter 6](chapter_6.html).
...
state of a `DynamicArray`, but we *can* check its observable behaviour.
