...
First, we define an abstract type that encapsulates the *reply
 ...
 * `getValue cnt` for getting the value of the counter server,
...
that `threadDelay` accepts an argument in microseconds, so we have to multiply
...
use of sub-threads to ensure responsivity and robustness. We are
